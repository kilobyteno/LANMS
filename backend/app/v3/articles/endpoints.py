...
async def post_create_article(
    event_id: UUID7, article_data: ArticleCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)
) -> JSONResponse:
...
async def get_articles_list(event_id: UUID7, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)) -> JSONResponse:
...
async def get_all_articles_for_event(event_id: UUID7, db: Session = Depends(get_db)) -> JSONResponse:
...
async def get_article_by_id(event_id: UUID7, article_id: UUID7, db: Session = Depends(get_db)) -> JSONResponse:
    """Get article by ID"""
...
async def put_update_article(
    event_id: UUID7, article_id: UUID7, article_data: ArticleUpdate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)
) -> JSONResponse:
    """Update article"""
